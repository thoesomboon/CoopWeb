 ...
 */
	...
	$.jstree.defaults.contextmenu = {
		 ...
		 * * `intcut` - keyCode which will trigger the action if the menu is open (for example `113` for rename, which equals F2)
		 * * `intcut_label` - intcut label (like for example `F2` for rename)
		 *
		...
		$.vakata.context = {
			...
			_parse : function (o, is_callback) {
				...
				$.each(o, function (i, val) {
					...
					sep = false;
					str += "<"+"li class='" + (val._class || "") + (val._disabled === true || ($.isFunction(val._disabled) && val._disabled({ "item" : val, "reference" : vakata_context.reference, "element" : vakata_context.element })) ? " vakata-contextmenu-disabled " : "") + "' "+(val.intcut?" data-intcut='"+val.intcut+"' ":'')+">";
					...
					str += ($.isFunction(val.label) ? val.label({ "item" : i, "reference" : vakata_context.reference, "element" : vakata_context.element }) : val.label) + (val.intcut?' <span class="vakata-contextmenu-intcut vakata-contextmenu-intcut-'+val.intcut+'">'+ (val.intcut_label || '') +'</span>':'') + "<"+"/a>";
					if(val.submenu) {
		...
		$(function () {
			...
			vakata_context.element
				...
				.on('keydown', function (e) {
					e.preventDefault();
					var a = vakata_context.element.find('.vakata-contextmenu-intcut-' + e.which).parent();
