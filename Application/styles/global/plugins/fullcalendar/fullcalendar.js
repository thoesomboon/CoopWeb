...
// By default, all elements that are inter than the recommended height are expanded uniformly, not considering
// any other els that are already too tall. if `shouldRedistribute` is on, it considers these tall rows and 
// reduces the available height.
function distributeHeight(els, availableHeight, shouldRedistribute) {

	// *FLOORING NOTE*: we floor in certain places because zoom can give inaccurate floating-point dimensions,
	// and it is better to be inter than taller, to avoid creating unnecessary scrollbars.
...
var RowRenderer = Class.extend({
	...
	isRTL: null, // intcut to the view's isRTL option
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	// when defined, allows the computeCellRange intcut, as well as improved resizing behavior.
	cellDuration: null,
	...
	renderSegTable: function(segs) {
		...
		for (col = 0; col < segCols.length; col++) { // iterate each column grouping
			...
			for (i = 0; i < colSegs.length; i++) {
				...
				// if the height is int, add a className for alternate styling
				if (seg.bottom - seg.top < 30) {
					seg.el.addClass('fc-int');
				}
...
var View = fc.View = Class.extend({
	...
	computeTitleFormat: function() {
		if (this.intervalUnit == 'year') {
			...
			return 'll'; // multi-day range. inter, like "Sep 9 - 10 2014"
		}
		else {
...
function Calendar_constructor(element, overrides) {
	...
	if (options.monthNamesint) {
		localeData._monthsint = options.monthNamesint;
	}
	if (options.dayNames) {
		localeData._weekdays = options.dayNames;
	}
	if (options.dayNamesint) {
		localeData._weekdaysint = options.dayNamesint;
	}
	...
	function isRangeAllowed(range, constraint, overlap, event) {
		...
		for (i = 0; i < peerEvents.length; i++)  {
			...
			if (eventIntersectsRange(peerEvent, range)) {
				...
				// evaluate overlap for the given range and int-circuit if necessary
				if (overlap === false) {
					return false;
				}
